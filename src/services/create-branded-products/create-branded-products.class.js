...
class Service {
    ...
    async create(data, params) {
        ...
        if (data.PRODUCT_TYPE === 'DRUG') {
            ...
            delete product._id;
            let uploadedProductImg = {};
            if (data.BNBase64 !== '') {
                uploadedProductImg = await imgProductUploadService.create({
                    'uri': data.BNBase64
                });
                (host === 'localhost') ? host = `http://${host}:${port}`: host;
                product.URL = `${host}/img/products/${uploadedProductImg.id}`;
            }
            ...
            return jsend.fail('Sorry; Presently Formulary can only save Drug-type product');
        }
